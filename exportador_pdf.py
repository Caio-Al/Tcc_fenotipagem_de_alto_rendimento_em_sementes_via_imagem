...
class ExportadorRelatorio:
    def gerar_pdf(self, lista_sementes, nome_arquivo="resultados/relatorio_fenotipagem.pdf"):
        ...
        headers = ["ID", "Area (px)", "Perim. (px)", "Compr. (px)", "Larg. (px)", "Circularid.", "Razao Asp.", "Cor (RGB)"]
        ...
        for s in lista_sementes:
            ...
            pdf.cell(0, 5, txt=f"Area: {s.area:.2f} px | Perimetro: {s.perimetro:.2f} px", ln=True)
            
            pdf.set_x(pos_x_texto)
            pdf.cell(0, 5, txt=f"Comprimento: {s.comprimento:.2f} px | Largura: {s.largura:.2f} px", ln=True)
